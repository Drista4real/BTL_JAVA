...
public class DoctorHospitalRoomPanel extends JPanel {
    ...
    private void loadRoomsFromDatabase() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            // In ra các tên cột có trong kết quả để debug
            java.sql.ResultSetMetaData metaData = rs.getMetaData();
            int columnCount = metaData.getColumnCount();
            System.out.println("Danh sách các cột trong bảng HospitalRooms:");
            ...
            while (rs.next()) {
                ...
                // Sửa lỗi: Sử dụng RoomID thay cho RoomNumber vì RoomNumber không tồn tại
                HospitalRoom room = new HospitalRoom(
                        rs.getString("RoomID"), // Thay RoomNumber bằng RoomID
                        roomType,
                        rs.getInt("TotalBeds"),
                        0.0, // PricePerDay không có trong bảng, để mặc định là 0.0
                        "Chưa có", // Department không có trong bảng, để mặc định
                        String.valueOf(rs.getInt("FloorNumber")),
                        "Chưa có" // Building không có trong bảng, để mặc định
                );
                ...
                // Cẩn thận với các phương thức này, nếu chúng cũng sử dụng các cột không tồn tại
                try {
                    loadBedsForRoom(conn, room);
                    loadUsageHistoryForRoom(conn, room);
                } catch (SQLException e) {
                    System.out.println("Cảnh báo: " + e.getMessage());
                    // Không dừng tiến trình nếu không tải được các thông tin phụ
                }
