...
public class DoctorPrescriptionDetailPanel extends JPanel {
    ...
    public DoctorPrescriptionDetailPanel(User user) {
        this.currentUser = user;
        this.prescriptionDetailList = new ArrayList<>();
        this.currentDetail = null;

        initializeUI();
        loadDetailsFromDB(null); // Tải tất cả chi tiết
    }

    public DoctorPrescriptionDetailPanel(User user, List<PrescriptionDetail> details) {
        this.currentUser = user;
        this.prescriptionDetailList = new ArrayList<>(details);
        this.currentDetail = details.isEmpty() ? null : details.get(0);
    ...
    private void loadDetailsFromDB(String prescriptionId) {
        ...
        try (Connection conn = DriverManager.getConnection(
                ...
                "root", "050705")) {
            String query = prescriptionId == null ?
                    "SELECT DetailID, PrescriptionID, MedicationID, Dosage, Instructions, Price, Quantity FROM PrescriptionDetails" :
                    "SELECT DetailID, PrescriptionID, MedicationID, Dosage, Instructions, Price, Quantity FROM PrescriptionDetails WHERE PrescriptionID = ?";
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
                        ...
                        prescriptionDetailList.add(detail);
                    }
                }
            }
    ...
    private void updateTableData() {
        ...
        if (detailTable.getRowCount() > 0) {
            ...
            currentDetail = prescriptionDetailList.get(0);
            updateDetailInfo();
        } else {
            currentDetail = null;
            updateDetailInfo();
    ...
    private void editDetail() {
        ...
        if (updatedDetail != null) {
            try {
                updateDetailInDB(updatedDetail);
    ...
    private void deleteDetail() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                deleteDetailFromDB(currentDetail);
    ...
    private void saveDetail(PrescriptionDetail detail) throws SQLException {
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/PatientManagement?allowPublicKeyRetrieval=true&useSSL=false",
                "root", "050705")) {
            String query = "INSERT INTO PrescriptionDetails (DetailID, PrescriptionID, MedicationID, Dosage, Instructions, Price, Quantity) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                stmt.setString(1, detail.getDetailId());
                stmt.setString(2, detail.getPrescriptionId());
                stmt.setString(3, detail.getMedicationId());
                stmt.setString(4, detail.getDosage());
                stmt.setString(5, detail.getInstructions());
                stmt.setDouble(6, detail.getPrice());
                stmt.setInt(7, detail.getQuantity());
                stmt.executeUpdate();
            }
        }
    }

    private void updateDetailInDB(PrescriptionDetail detail) throws SQLException {
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/PatientManagement?allowPublicKeyRetrieval=true&useSSL=false",
                "root", "050705")) {
            String query = "UPDATE PrescriptionDetails SET PrescriptionID = ?, MedicationID = ?, Dosage = ?, Instructions = ?, Price = ?, Quantity = ? " +
                    "WHERE DetailID = ?";
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                stmt.setString(1, detail.getPrescriptionId());
                stmt.setString(2, detail.getMedicationId());
                stmt.setString(3, detail.getDosage());
                stmt.setString(4, detail.getInstructions());
                stmt.setDouble(5, detail.getPrice());
                stmt.setInt(6, detail.getQuantity());
                stmt.setString(7, detail.getDetailId());
                stmt.executeUpdate();
            }
        }
    }

    private void deleteDetailFromDB(PrescriptionDetail detail) throws SQLException {
        try (Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/PatientManagement?allowPublicKeyRetrieval=true&useSSL=false",
                "root", "050705")) {
            String query = "DELETE FROM PrescriptionDetails WHERE DetailID = ?";
            try (PreparedStatement stmt = conn.prepareStatement(query)) {
                stmt.setString(1, detail.getDetailId());
                stmt.executeUpdate();
            }
